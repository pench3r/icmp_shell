...
void des_process(unsigned char *data, int data_len, int mode, unsigned char *output, int *output_len) {
	...
	unsigned char des_key[] = {0x29, 0x08, 0x6c, 0x3a, 0xd0, 0xbe, 0xd0, 0xb4};
	unsigned char *data_ptr = data;
	unsigned char *output_ptr = output;
	int proc_total_bytes = 0;
	// Generate DES key set
	int process_mode = mode;
	...
	while(block_count < number_of_blocks) {
		...
		if (block_count == number_of_blocks) {
			if (file_size % 8) {
				memcpy(data_block, data_ptr, (file_size % 8));
			} else {
				memcpy(data_block, data_ptr, 8);
			}	
			// encrypt
			if (process_mode == ENCRYPTION_MODE) {
				...
				memcpy(output_ptr, processed_block, 8);
				output_ptr += 8;
				proc_total_bytes += 8;
			...
			// decrypt
				...
				if (padding < 8) {
					memcpy(output_ptr, processed_block, 8 - padding);
					proc_total_bytes += 8 - padding;
				}
			}
		} else {
			memcpy(data_block, data_ptr, 8);
...
void encrypt_msg(unsigned char *plain, int plain_len, unsigned char *cipher, int *cipher_len) {
	des_process(plain, plain_len, ENCRYPTION_MODE, cipher, cipher_len);
}

void decrypt_msg(unsigned char *cipher, int cipher_len, unsigned char *plain, int *plain_len) {
	des_process(cipher, cipher_len, DECRYPTION_MODE, plain, plain_len);
}
